...
# pylint: disable=arguments-differ,too-many-arguments
# pylint: disable=too-many-instance-attributes, unused-argument
class ConditionalRecurrentDecoder(Decoder):
    """A conditional RNN decoder with attention."""
    ...
    def __init__(self,
        ...
        super(ConditionalRecurrentDecoder, self).__init__()
        ...
        self.rnn2 = rnn(encoder.output_size, hidden_size, num_layers,
                        batch_first=True,
                        dropout=dropout if num_layers > 1 else 0.)

        # combine output with context vector before output layer (Luong-style)
        self.att_vector_layer = nn.Linear(
            hidden_size + encoder.output_size + emb_size, hidden_size, bias=True)
    ...
    def _forward_step(self,
        ...
        # rnn_input: batch x 1 x emb+2*enc_size
        _, hidden_intermediate = self.rnn(rnn_input, hidden)

        # use new (top) decoder layer as attention query
        if isinstance(hidden, tuple):
            query = hidden_intermediate[0][-1].unsqueeze(1)
        else:
            # [#layers, B, D] -> [B, 1, D]
            query = hidden_intermediate[-1].unsqueeze(1)
        ...
        _, hidden = self.rnn2(context, hidden_intermediate)

        # combine context with decoder hidden state before prediction
        att_vector_input = torch.cat([query, context, prev_embed], dim=2)
        # batch x 1 x 2*enc_size+hidden_size+emb_size
        att_vector_input = self.hidden_dropout(att_vector_input)
